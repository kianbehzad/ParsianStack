...
public:
  ...
  ~GLText();
  void drawString(rcsc::Vector2D loc, double angle, double size, const char* str, GLText::HAlignOptions hAlign=LeftAligned, GLText::VAlignOptions vAlign=MiddleAligned);
  void drawGlyph(char glyph);
  void initializeGlyph(char ch);
  double getWidth(char ch);
  double getHeight(char ch);
  rcsc::Vector2D getSize(char ch);
