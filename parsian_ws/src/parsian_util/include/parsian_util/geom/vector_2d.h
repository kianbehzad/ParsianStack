...
#include <parsian_msgs/msg/vector2_d.hpp>
...
public:
    ...
    */
    Vector2D(const parsian_msgs::msg::Vector2D& vec)
        : x(vec.x)
        , y(vec.y) {
    }
    ...
    */
    parsian_msgs::msg::Vector2D toParsianMessage() const {
        parsian_msgs::msg::Vector2D vec;
        vec.x = this->x;
        vec.y = this->y;
        return vec;
    }
